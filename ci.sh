...
do
  ...
  RUST_BACKTRACE=0 QUICKCHECK_TESTS=$(expr ${QUICKCHECK_TESTS} / 50) cargo test --all-features
  RUST_BACKTRACE=0 QUICKCHECK_TESTS=$(expr ${QUICKCHECK_TESTS} / 10) cargo test -r --all-features
  RUST_BACKTRACE=0 QUICKCHECK_TESTS=$(expr ${QUICKCHECK_TESTS} / 10) cargo test -r --all-features --examples
done
